  ...
  difficulties.forEach(difficulty => {
    for (let i = 1; i <= 20; i++) {
      questions.push({
        ...
        careerSlug,
        difficulty,
        questionText: `Sample ${difficulty} technical question ${i} regarding the core principles and common challenges faced in ${careerName} roles.`,
        options: [
          { id: 'a', text: `Option A: A standard methodology for ${careerName}s.` },
          { id: 'b', text: `Option B: The correct architectural approach for ${difficulty} scenarios.` },
          { id: 'c', text: `Option C: An anti-pattern commonly avoided.` },
          { id: 'd', text: `Option D: A legacy tool no longer in use.` }
        ],
        correctAnswer: 'b',
        explanation: `Option B is correct because it addresses the specific ${difficulty} constraints required for a ${careerName} to optimize performance and scalability.`
      });
    }
  });
