...
function CVSection({ title, children, sectionClass, dark = false }) {
  return <section className="mt-7"><h3 className={`mb-3 text-sm font-extrabold uppercase tracking-[0.16em] ${dark ? 'text-white/80' : 'text-slate-500'} ${sectionClass}`}>{title}</h3><div className={`text-sm leading-7 ${dark ? 'text-white/85' : 'text-slate-700'}`}>{children}</div></section>;
}
...
function SkillPills({ text, dark = false }) {
  return <div className="flex flex-wrap gap-2">{String(text).split(',').map((skill) => skill.trim()).filter(Boolean).map((skill) => <span key={skill} className={`rounded-md px-2 py-1 text-xs font-bold ${dark ? 'bg-white/10 text-white' : 'bg-slate-100 text-slate-700'}`}>{skill}</span>)}</div>;
}
