...
migrate((app) => {
  // Remove the 5000 char max limit on blogArticles content field
  const collection = app.findCollectionByNameOrId("blogArticles");
  const contentField = collection.fields.getByName("content");
  if (contentField) {
    contentField.max = 0;
    app.save(collection);
    console.log("[fix] Removed content max limit from blogArticles.");
  }

  // Re-seed blog articles that failed due to content length
  const articles = [
    {
  ...
  for (const article of articles) {
    try {
      ...
      if (existing.length > 0) {
        // Delete and re-add to update content
  ...
  console.log(`Blog fix migration complete: ${added} articles added.`);
});
